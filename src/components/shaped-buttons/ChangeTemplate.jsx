...
const ChangeTemplate = React.forwardRef(({ onClick, name, height }, ref) => {
  const style = {
    maxHeight: height,
    minHeight: height
  }
  ...
  return (
    <div className={styles.container} style={style} onClick={() => onClick()} ref={ref}>
      <svg width={264} height={height} viewBox={`0 0 264 ${height - 2}`} fill='none' xmlns='http://www.w3.org/2000/svg' className={styles.svg}>
        {height === DEFAULT_HEIGHT_TEMPLATE
          ...
          : (
            <>
              <rect x={0.5} y={1.02789} width={263} height={height - 2} rx={3.5} fill='none' className={styles.rectFilled} />
              <rect x={0.5} y={1.02789} width={263} height={height - 2} rx={3.5} stroke='none' className={styles.rectBordered} />
            </>
            )}
      </svg>
