...
const Plugin = React.forwardRef(({ onClickSort, onClickRemove, index, name, height, sortable }, ref) => {
  const style = {
    maxHeight: height,
    minHeight: height
  }
  return (
    <div className={styles.container} style={style} ref={ref}>
      <svg width={264} height={height} viewBox={`0 0 264 ${height - 1}`} fill='none' xmlns='http://www.w3.org/2000/svg' className={styles.svg}>
        {index === 0
          ? <path
          ...
          : (
            <>
              <rect x={0.5} y={0.61377} width={263} height={height - 1} rx={3.5} fill='none' className={styles.rectFilled} />
              <rect x={0.5} y={0.61377} width={263} height={height - 1} rx={3.5} stroke='none' className={styles.rectBordered} />
            </>
            )}
      </svg>
